...
{
    return crc == CalcCrc8(buffer, len);
}

int CheckCrc16(uint8_t *buffer, uint32_t len, uint16_t crc)
{
    uint16_t calcCrc = 0;
    calcCrc = CalcCrc16(buffer, len);
...
{
    return crc == CalcCrc32(buffer, len);
}
...
uint16_t CalcCrc16(const uint8_t *buf, uint32_t bytes)
{
    ...
    if (ptr != NULL) {
        for (a = 0; a < bytes; a++) {
            crc = (crc >> 8) ^ crcTable16[(crc ^ (uint16_t)*ptr++) & 0x00FF];
        }
    }
...
uint32_t CalcCrc32(const uint8_t *buf, uint32_t bytes)
{
    ...
    if (ptr != NULL) {
        for (a = 0; a < bytes; a++) {
            crc = (crc >> 8) ^ crcTable32[(crc ^ (uint32_t)*ptr++) & 0x000000FFul];
        }
    }
...
{
    /* Find need to be a multiple of 2*/
    uint32_t sum = 0;
    uint32_t cnt = 0;

    /* make words out of every two adjacent bytes*/
    for (cnt = 0; (cnt + 1) < size; cnt += 2)
        sum = sum +
        ((((uint32_t)data[cnt]) & 0xFF) << 8) +
        (((uint32_t)data[cnt + 1]) & 0xFF);
